...
{
    public partial class frmRomStubCreator : Form
    {
        public frmRomStubCreator()
        {
            InitializeComponent();
        }
        ...
        private void btnAction_Click(object sender, EventArgs e)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (rbPrefix.Checked)
                {
                    finalName = string.Format("{0};{1}.{2}.{3}", tag, name, extension, overrideExtension);
                }
                else
                {
                    finalName = string.Format("{0}.{1};{2}.{3}", name, extension, tag, overrideExtension);
                }

                string finalPath = System.IO.Path.Combine(destionation, finalName);

                System.IO.File.Create(finalPath).Dispose();
